...
//   db로 데이터 보내기
  ...
  const handleOptionSelect = () => {
    ...
    const colorCharge = parseInt(document.querySelector('.color-option option:checked')?.dataset.chrg || 0);
    const leatherCharge = parseInt(document.querySelector('.leather-option option:checked')?.dataset.chrg || 0);
    const stoolCharge = parseInt(document.querySelector('.stool-option option:checked')?.dataset.chrg || 0);
  
    const optionPrice = colorCharge + leatherCharge + stoolCharge;
    const basePrice = parseInt(document.querySelector('.final-price').dataset.chrg);
