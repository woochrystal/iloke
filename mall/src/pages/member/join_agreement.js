...
import styles from './join_agreement.module.css'
...
function JoinAgreement(){
    ...
    // 개별 동의 체크 핸들러
    const handleEachAgree = (setter) => (checked) => {
      setter(checked);
      setAgreeAll(checked && agree1 && agree2 && agree3); // 전체 동의 상태 업데이트
      setShowWarning(false);
    };
    ...
    return(
        <div className={styles.agreeContainer}>
            ...
            <div className={styles.tabs}>
                <div className={`${styles.tab} ${styles.active}`}>약관동의</div>
                <div className={styles.tab}>정보입력</div>
                <div className={styles.tab}>가입완료</div>
            </div>
            <div className={styles.termsContainer}>
                <div className={styles.termsHeader}>
                    <input type="checkbox" id="agree-all" checked={agreeAll} onChange={(e) => handleAllAgree(e.target.checked)}/>
                    <label htmlFor="agree-all">이로케의 모든 약관을 확인하고 전체 동의함</label>
                    <br/>
                    <p>(전체동의는 선택항목도 포함됩니다)</p>
                </div>
                <br/>

                {/* Terms Section 1 */}
                <div className={styles.termsSection}>
                    <input type="checkbox" id="agree1" checked={agree1} onChange={(e) => handleEachAgree(setAgree1)(e.target.checked)}/>
                    <label htmlFor="agree1">(필수) 이용약관 동의</label>
                    {/*<Link to="#" onclick="loadContent('./sub/service/agreement1.html')">전체보기</Link>*/}
                    <div className={styles.termsTextbox}>
						제1조(목적)<br/>
                        <br/>
                        이 약관은 (주)자코모 회사(전자거래 사업자)가 운영하는 자코모 사이버 몰(이하 "몰"이라 한다)에서 제공하는 인터넷 관련 서비스(이하 "서비스"라 한다)를 이용함에 있어 사이버몰과 이용자의 권리·의무 및 책임사항을 규정함을 목적으로 합니다.<br/>
                        ※ 「PC통신등을 이용하는 전자거래에 대해서도 그 성질에 반하지 않는한 이 약관을 준용합니다」<br/>
                        <br/>
                        <br/>
                        제2조(정의)<br/>
                        <br/>
                        ① "몰"이란 (주)자코모 회사가 재화 또는 용역을 이용자에게 제공하기 위하여 컴퓨터등 정보통신설비를 이용하여 재화 또는 용역을 거래할 수 있도록 설정한 가상의 영업장을 말하며, 아울러 사이버몰을 운영하는 사업자의 의미로도 사용합니다.<br/>
                        ...
                        ② 이용자는 다음 각 호의 어느 하나에 해당하는 경우에는, '몰'의 의사에 반하여 제1항에 따른 청약철회등을 할 수 없습니다.<br/>
                        ...
                        5. 용역 또는 '문화산업진흥 기본법' 제2조 25호의 디지털콘텐츠의 제공이 개시된 경우. 다만, 가분적 용역 또는 가분적 디지털콘텐츠로 구성된 계약의 경우에는 제공이 개시되지 아니한 부분에 대하여는 그러하지 아니하다<br/>
                        ...
                        <br/>
                        제16조(청약철회 등의 제한)<br/>
                        약관 제15조 제2항 제6호의 “대통령령으로 정하는 경우”란 소비자의 주문에 따라 개별적으로 생산되는 재화등 또는 이와 유사한 재화등에 대하여 본 약관 제15조 제2항 제5호에 따른 청약철회등(이하 “청약철회등”이라 한다)을 인정하는 경우 '몰'(통신판매업자)에게 회복할 수 없는 중대한 피해가 예상되는 경우로서 사전에 해당 거래에 대하여 별도로 그 사실을 고지하고 소비자의 서면(전자문서를 포함한다)에 의한 동의를 받은 경우를 말한다.<br/>
                        <br/>
                        <br/>
                        제17조(청약철회 등의 효과)<br/>
                        ...
                        <br/>
                        제21조(이용자의 의무) 이용자는 다음 행위를 하여서는 안 됩니다.<br/>
                        ...
                        <br/>
                        <br/>
                        부칙 : 주문 생산되는 제품에 대한 고지<br/>
                        <br/>
                        자코모 소파는 전자상거래 등 에서의 소비자보호에 관한 법률 시행령 제21조(청약철회등의 제한),본 약관 제16조에 의한 '소비자의 주문에 따라 개별적으로 생산되는 재화'에 해당합니다. 따라서 동법 제17조 제2항 제6호, 본 약관 제15조 제2항 제6호에 해당되어, 소비자의 요청에 따른 청약의 철회는 제한됨을 알려드립니다.<br/>
                        또한 소비자의 귀책 사유에 따른 계약해제로 인한 반품 및 교환 신청의 경우, 동법 제19조에서 정하는 범위 내의 손해배상을 청구할 수 있음을 알려드립니다.<br/>
                        <br/>
                        <br/>
                        본 약관은 2018년 1월 22일부터 적용됩니다.
                    </div>
                </div>

                {/* Terms Section 2 */}
                <div className={styles.termsSection}>
                    <input type="checkbox" id="agree2" checked={agree2} onChange={(e) => handleEachAgree(setAgree2)(e.target.checked)}/>
                    <label htmlFor="agree2">(필수) 개인정보 수집 및 이용 동의</label>
                    {/*<Link to="#" onclick="loadContent('./sub/service/agreement2.html')">전체보기</Link>*/}
                    <div className={styles.termsTextbox}>
						개인정보 수집 및 이용은 회원가입, 고객상담, 본인여부 확인, 주문/결제 및 배송 서비스 이용에 있다.<br/>
                        각 이용 목적과 그 항목, 기간은 아래로 정의된다.<br/>
                        <br/>
                        - 수집 및 이용 목적 : 이용자 식별 및 본인여부 확인<br/>
                        - 수집 항목 : 이름, 아이디, 비밀번호<br/>
                        - 보유 및 이용기간 : 회원탈퇴 후 5일까지<br/>
                        <br/>
                        - 수집 및 이용 목적 : 민원 등 고객 고충처리<br/>
                        - 수집 항목 : 이메일, 휴대폰번호<br/>
                        - 보유 및 이용기간 : 회원 탈퇴 후 5일까지<br/>
                        <br/>
                        - 수집 및 이용 목적: 주문/결제 시 상품 배송<br/>
                        - 수집 항목: 구매자 정보, 상품 구매/취소/반품/교환/환불 정보, 수령인 정보<br/>
                        - 보유 및 이용기간: 회원 탈퇴 후 5일까지<br/>
                        <br/>
                        - 수집 및 이용 목적 : 만 14세 미만 아동 확인<br/>
                        - 수집 항목 : 법정 생년월일<br/>
                        - 보유 및 이용기간 : 회원 탈퇴 후 5일까지
                    </div>
                </div>

                {/* Terms Section 3 */}
                <div className={styles.termsSection}>
                    <input type="checkbox" id="agree3" checked={agree3} onChange={(e) => handleEachAgree(setAgree3)(e.target.checked)}/>
                    <label htmlFor="agree3">(선택) 마케팅 정보 수신 및 활용 동의</label>
                    {/*<Link to="#" onclick="loadContent('./sub/service/agreement3.html')">전체보기</Link>*/}
                    <div className={styles.termsTextbox}>
                        (주)자코모에서 제공하는 이벤트 및 혜택 등 다양한 정보를 문자, 이메일 등으로 받아보실 수 있습니다.<br/>
                        마케팅 정보 수신 및 활용 동의 여부와 관계없이 회원가입 및 서비스를 이용하실 수 있습니다.<br/>
                        또한 서비스의 중요 안내 사항 및 주문/배송에 대한 정보는 마케팅 정보 수신 동의 여부와 관계없이 발송됩니다.<br/>
                        <br/>
                        1. 수집 및 이용 목적<br/>
                        - 인구 통계학적 특성에 따른 서비스 제공 및 광고 게재<br/>
                        - 접속, 이용 및 방문 빈도 파악, 서비스 이용에 대한 통계 등 업무와 관련된 각종 통계 자료의 작성 및 서비스 개발<br/>
                        - 신규 서비스 개발과 이벤트 행사에 따른 정보 전달 및 맞춤 서비스 제공<br/>
                        - 제품 안내, 홍보 및 이벤트 등의 마케팅, 광고성 정보 전달<br/>
                        <br/>
                        2. 수집 및 이용 항목<br/>
                        - 회원 정보(이름, 휴대폰번호, 이메일, 성별, 생년월일, 가입일시)<br/>
                        - 구매 및 배송 서비스 이용 정보(장바구니 상품, 구매일시, 주소, 받으시는 곳)<br/>
                        - 수집 및 이용 항목은 마케팅 및 분석, 프로모션의 목적에 따라 달라질 수 있으며 수집 시점에 동의 여부를 별도 확인함<br/>
                        <br/>
                        3. 보유 및 이용기간<br/>
                        - 서비스 이용 과정 및 사업처리과정에서 수집될 수 있는 개인정보는 3년으로 한다.<br/>
                        - 단, 정보 삭제 또는 이용 정지 요청 및 회원 탈퇴 시 보유 가능 기간은 5일까지다.
                    </div>
                </div>
