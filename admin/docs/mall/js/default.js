...
// /////////최근 검색어
// $(".ns-box label").click(function(e){
//     e.preventDefault();  // 기본 기능 중지
//     e.stopPropagation();

//     let ns_list = $('#nav_search').val().replace(/\s+/g, ' ').trim();

//     if(ns_list.length === 0){//내용 없을 떄
//         alert('입력된 내용이 없습니다.');
//         $('#nav_search').val('');//리셋
//     }else if(ns_list.length >= 20){//글자수 제한
//         alert('20자 이하로 검색해주세요.');
//         $('#nav_search').val('');//리셋
//     }else{

//         let ttt = '<li><a href="#">'+ ns_list +'</a>';
//             ttt+='<i class="fa-solid fa-circle-xmark ns_remove"></i></li>';//li추가

//         $('#latest_rank').prepend(ttt);//오래된 검색어가 아래에 오게
        
//         if($('#latest_rank li').length > 5){ //검색어 5개 이상일 때
//             $('#latest_rank li')[5].remove();//제일 오래된 검색어부터 삭제
//         }
//         $('#nav_search');//리셋
//     }
// });
...
//     $(this).parent('li').remove().trim(); // 클릭된 li 요소 제거
// });
