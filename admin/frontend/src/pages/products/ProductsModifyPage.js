...
function ProductsModifyPage({ id, myData, code }) {
  ...
  useEffect(() => {
    ...
    selectProductDetail(id)
      .then((res) => {
        const data = res.data;
        setProlist(data);
        setSelectedOptions(data.selectedOptions); // 기존 옵션 설정
        setSelectedKeywords(data.selectedKeywords); // 기존 키워드 설정
        setIsLoading(false); //로딩완료
      })
      .catch((err) => {
        console.error("상품 리스트 백엔드 연결 실패", err);
        setIsLoading(false); //디비 연결 실패해도 로딩완료
      });
  }, [id]);
  const product = proList.ret; //정보 불러오기
  console.log(product);
  ...
  function getSelectedDetails() {
    ...
    if (!selectedOptions || !Array.isArray(selectedOptions)) {
      console.error("selectedOptions는 유효한 배열이 아닙니다.");
      return []; // 배열이 아니면 빈 배열 반환
    }
  
    if (!selectedKeywords || !Array.isArray(selectedKeywords)) {
      console.error("selectedKeywords는 유효한 배열이 아닙니다.");
      return []; // 배열이 아니면 빈 배열 반환
    }
  
    if (!opVal || !Array.isArray(opVal)) {
      console.error("opVal는 유효한 배열이 아닙니다.");
      return []; // 배열이 아니면 빈 배열 반환
    }
  
    if (!keyVal || !Array.isArray(keyVal)) {
      console.error("keyVal는 유효한 배열이 아닙니다.");
      return []; // 배열이 아니면 빈 배열 반환
    }

    
    const optionDetails = selectedOptions
      .map((option) => {
        const optionData = opVal.find((opt) => opt.name === option);
        if (!optionData) {
          console.warn(`옵션 데이터가 없습니다: ${option}`);
          return null; // 데이터가 없으면 null 처리
        }
        return {
    ...
    const keywordDetails = selectedKeywords
      .map((keyword) => {
        ...
        if (!keywordData) {
          console.warn(`키워드 데이터가 없습니다: ${keyword}`);
          return null; // 데이터가 없으면 null 처리
        }
        return {
          ...
          <div className="join_base_wrap">
            ...
            <table>
              <tbody>
                ...
                {option02 && Array.isArray(opVal) && opVal.length > 0 && ( // 선택된 상위 옵션이 있을 경우에만 실행
                  <tr>
                        ...
                        opVal.map((st, i) => (
                          <label key={i}>
                            <input
                              ...
                              name="option"
                              value={st.name}
                              checked={Array.isArray(selectedOptions) && selectedOptions.includes(st.name)}
                              onChange={checkClick}
                            />
                            {st.name}
                          </label>
                ...
                {keyword02 && Array.isArray(keyVal) && keyVal.length > 0 && ( // 선택된 상위 키워드가 있을 경우에만 실행
                  <tr>
                        ...
                        keyVal.map((st, i) => (
                          <label key={i}>
                            <input
                              ...
                              name="keyword"
                              value={st.name}
                              checked={Array.isArray(selectedKeywords) && selectedKeywords.includes(st.name)}
                ...
                <tr>
                  ...
                  <td>
                    <textarea name="benefit" defaultValue={product.benefit} />
                  </td>
                </tr>
                <tr>
                  <td className="inputHead">
...
export default ProductsModifyPage;
