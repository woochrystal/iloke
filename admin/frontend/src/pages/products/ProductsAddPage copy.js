import { insertProduct, selectOption, selectOptionVal, selectKeyword ,selectKeywordVal } from '../../services/api';  // API 호출 함수
...
function ProductsAddPage({myData}) {
    ...
    //옵션값 불러오기
    useEffect(() => {
        if (option02) {  // 상위 옵션이 선택된 경우에만 실행
            ...
            if (selectedOption) {
                ...
                // console.log(code)
                selectOptionVal(selectedOption.code)
                    .then(res => {
...
///////////////////////////////
    ...
    const [selectedKeywords, setSelectedKeywords] = useState([]); // 선택된 하위 키워드들
    ...
    //키워드값 불러오기
    useEffect(() => {
        if (keyword02) {  // 상위 키워드이 선택된 경우에만 실행
            // 키워드 리스트에서 선택된 keyword02의 code 값을 찾아서 전달
            const selectedKeyword =  keyword01.find(function(key) {
                                        // keyword02와 일치하는 name 찾기
                                        return key.name === keyword02
                                    });
            if (selectedKeyword) {
                const { code } = selectedKeyword  // 선택된 키워드의 code 값 추출
                // console.log(code)
                selectKeywordVal(selectedKeyword.code)
                    .then(res => {
                ...
                <div className="join_base_section memberList">
                    <div className="join_base_wrap">
                        ...
                        <table>
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type='file' name='upMultiFiles' onChange={upMultiFiles} multiple/>
                                    </td>
                                </tr>
                                <tr>
                                    <td className="inputHead">
                                                ...
                                                opVal.map((st, i) => (
                                                    <label key={i}>
                                                        <input  type="checkbox" 
                                                                name="option"
                                                                value={st.name} 
                                                                checked={selectedOptions.includes(st.name)}
                                                                onChange={checkClick} />
                                                        {st.name}
                                                    </label>
                                                ))
                                                ...
                                                keyVal.map((st, i) => (
                                                    <label key={i}>
                                                        <input  type="checkbox" 
                                                                name="keyword"
                                                                value={st.name} 
                                                                checked={selectedKeywords.includes(st.name)}
                        ...
                        <div className='btn-wrap'>
                            <button type="button" className="write-btn" onClick={goList}>상품목록</button>
                            <button type="button" className="home-btn" onClick={goHome}>홈</button>
                        </div>
                    </div>
                </div>
