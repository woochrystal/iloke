...
router.get('/option/value/:main_code',async (req,res)=>{
    const { main_code } = req.params;
    ...
    try {
      const [results] = await connection.execute(
        `SELECT 
            ...
            ov.price AS value_price,
            ov.status AS value_status
        FROM opkey_value ov
        JOIN option_keyword ok ON ov.main_code = ok.code
        WHERE ov.main_code = ?
        AND ov.status = 1`, 
        [main_code]
      );
    //   console.log('백엔드 응답:', results);
      res.json(results);
      
    } catch (error) {
      console.error(error);
      res.status(500).json({ error: 'Failed to fetch options with values 키워드연결실패' });
    }
})
