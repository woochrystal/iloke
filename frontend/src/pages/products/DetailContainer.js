...
import { insertOptionVal, selectOptionVal } from "../../services/api";
...
const DetailContainer = ({
  ...
  const [newValueRow, setNewValueRow] = useState(null); // 신규 옵션 값 행 추가 상태
  ...
  const handleSaveAll = () => {
    ...
    if (newValueRow) {
      ...
      savePromises.push(insertOptionVal(newRowWithMainCode));
    }

    updatedItems.forEach((item) => {
      const updatedItemWithMainCode = { ...item };
      savePromises.push(insertOptionVal(updatedItemWithMainCode));
    });
    ...
    Promise.all(savePromises)
      .then(() => {
        ...
        selectOptionVal(selectedHeadCode)
          .then((res) => {
